...
and a partition number in a parallel fashion making use of send, receive, scatter,
gather and reduce.
...
def batched(iterable: Iterable, iterable_type: Type, n: int or float) -> List[Iterable]:
    """
...
def riemann_sum_left(
    f: Callable[[(float | int)], float], x: List[(float | int)], delta_x: (float | int)
) -> float | int:
    """
...
def main():
    ...
    f = (
        function_string
    ) = a = b = n = delta_x = local_interval = batched_x = global_sum = None
